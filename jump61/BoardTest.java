...
public class BoardTest {
    ...
    private boolean testEquals(Board b1, Board b2) {
        assert b1.size() == b2.size();
        for (int N = 0; N < b1.size() * b1.size(); N++) {
            if (!b1.get(N).equals(b2.get(N))) {
                return false;
            }
        }
        return true;
    ...
    public void testGetN() {
        Board A = new Board(6);
        A.set(3, 4, 4, RED);
        Square testSquare = Square.square(RED, 4);
        assertEquals(testSquare, A.get(15));
    }

    @Test
    public void testNumPieces() {
        Board A = new Board(6);
        assertEquals(36, A.numPieces());
    }
    ...
    public void testCopy() {
        Board ori = new Board(3);
        ori.addSpot(BLUE, 1);
        ori.addSpot(RED, 8);
        ori.addSpot(RED, 2);
        Board blank = new Board(2);
        blank.copy(ori);
        assertTrue(testEquals(blank, ori));
        assertEquals(blank.getBoardStates(), ori.getBoardStates());
        ori.undo();
        assertFalse(testEquals(blank, ori));
        blank.undo();
    }
}
